...
@Component({
    selector: 'app-live-total-power-chart',
    templateUrl: './live-total-power-chart.component.html',
    styleUrls: ['./live-total-power-chart.component.less']
})
export class LiveTotalPowerChartComponent implements OnInit {

    constructor(private zone: NgZone) { }

    ngAfterViewInit() {
        this.zone.runOutsideAngular(() => {
            let chart = am4core.create("totalAllTruckOreGraph", am4charts.XYChart);
            ...
            for (i = 0; i <= 30; i++) {
                power -= Math.round((Math.random() < 0.5 ? 1 : -1) * Math.random() * 10);
                data.push({ date: new Date().setSeconds(i - 30), value: power });
            }
